...
RC_RSV_descriptiu$seasonal_catchup <- factor(RC_RSV_descriptiu$seasonal_catchup, levels = c("catchup","seasonal"))
...
RC_RSV_descriptiu$nirse_binary <- factor(RC_RSV_descriptiu$nirse_binary, levels = c(0,1), labels = c("no", "yes"))
...
#RC_RSV_seasonal <- RC_RSV_descriptiu
...
#RC_RSV_catchup <- RC_RSV_descriptiu
...
taula_biv <-table1(~ sex + months_old + month_of_birth + seasonal_catchup + ethnic + kindergarten + breast_binary + space_living + smokers_home + nirse_binary | region, data = RC_RSV_descriptiu, overall=F, extra.col=list(`p-value`=pvalue))
taula_biv_seasonal <-table1(~ sex + months_old + month_of_birth + ethnic + kindergarten + breast_binary + space_living + smokers_home + nirse_binary | region, data = RC_RSV_seasonal, overall=F, extra.col=list(`p-value`=pvalue))
taula_uv_catchup <-table1(~ region + sex + months_old + month_of_birth + ethnic + kindergarten + breast_binary + space_living + smokers_home + nirse_binary, data = RC_RSV_catchup)
